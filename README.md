...
The frontend should be served as static files from the folder used in the
deployment workflow:

```text
/home/jetty/deployment/site/eru-front
```
...
```caddy
app.eru-api.dk {
    root * /home/jetty/deployment/site/eru-front
    file_server
    try_files {path} /index.html
}
```

The `try_files` rule is important because the app uses React Router. It makes
refreshing routes such as `/feed`, `/explore`, and `/admin` work correctly.

The custom domain `app.eru-api.dk` points to the DigitalOcean droplet, while
`eru-api.dk` remains reserved for the backend API.
...
- Register a new user
- Login and refresh to confirm session restore
- Feed loads active production content
- `FACT`, `THEORY`, and `QUOTE` filters work
- Viewed content is tracked automatically
- Like, dislike, and bookmark can be saved and removed
- Like and dislike replace each other correctly
- Interaction overview shows liked, bookmarked, disliked, and viewed content
- Explore search/filter works
- Content detail pages open from Explore and Interactions
- Elaborate returns an AI-generated explanation
- Admin user can create, edit, search, page, and delete content
...
The backend is a separate Java/Javalin application using Hibernate ORM and
PostgreSQL. It owns authentication, authorization, persistence, and the OpenAI
integration used by the elaborate feature.
...
Only public frontend configuration belongs in `VITE_*` variables. Secrets such as
the OpenAI API key must stay in the backend environment.
